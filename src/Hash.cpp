...
uint64_t PosSearch256BitTable(uint64_t* p,uint64_t len,uint64_t* y,bit256KmerPara para)
{
	...
	uint64_t m=0;
	uint64_t r=0;
	if(cmp256BitKmer(y,p,wid)==0)
	{
		r=0;
	}
	else if(cmp256BitKmer((p+wid*(len-1)),y,wid)==0)
	{
		r=len;
	}
	else
	{
		if(len<MinSizeForSearch)
		{
			...
			{
				if(cmp256BitKmer(y,p+wid*i,wid)==0)
				{
					r=i;
					break;
				}
		...
		else
		{
			...
			{
				m=(s+e)/2;
				if(cmp256BitKmer(y,p+wid*m,wid)==0)
				{
					e=m;
				}
				else
...
/**********************original hashForFinal256BitCharKmer*********************/
...
void bit256insertHashFTable(struct NodeBit** pk,struct nodeBit c_tmp_hashtable,struct bit256KmerPara para)
{
	...
	Insert_ad_256bitKmer(pk+tmp,c_tmp_hashtable,para);
}
struct NodeBit * bit256Find_Minimal_Node_HashFTable(struct NodeBit** pk)
{
	uint32_t i=0;
	for(i=0;i<HashFSize;i++)
	{
		if(pk[i]!=NULL)
		{
			break;
	...
	return Find_Minimal_Node_bit(pk[i]);
}
void bit256linkHashFTable(struct NodeBit** pk)
{
	...
	if(i<HashFSize&&pk[i]!=NULL)
	{
		p_tmp=Find_Minimal_Node_bit(pk[i]);
		while(p_tmp->brother!=NULL)
		{
			p_tmp=p_tmp->brother;
		}
	}
	...
	for(uint32_t j=i+1;j<HashFSize;j++)
	{
		if(pk[j]!=NULL)
		{
			p_tmp->brother=Find_Minimal_Node_bit(pk[j]);
			while(p_tmp->brother!=NULL)
			{
				p_tmp=p_tmp->brother;
			}
